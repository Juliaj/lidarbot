...
{
	// Rate limit sensor reads to reduce I2C communication
	if (last_read_time_.nanoseconds() == 0 ||
	    (time - last_read_time_).seconds() >= READ_INTERVAL_) {
	...
	// Assign values to the state interfaces (always update, even with cached data)
